...
import { SET_PLAYER_TV, SET_PLAYER_FILM, SET_PLAYER_VOLUME, SET_PLAYER_EP_SEASONS } from "../types";
...
export const setPlayerTV = ({ id, title, src, backdrop, description, ep_title, ep_number, ep_season }) => {
	return async dispatch => {
		try {
			...
			const { seasons, episodes } = await Axios.post("http://localhost:8888/episodes/all", { id });

			dispatch({
				type: SET_PLAYER_EP_SEASONS,
				payload: { seasons, episodes },
			});
		} catch (error) {
